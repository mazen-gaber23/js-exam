...
function closeNav(){
   ...
   let ingr = [];
        ...
        function getIngredient(){
            let cartuna = '';
            for(let i = 0; i < 20; i++){
                let description = ingr[i].strDescription;
                description = description.substring(0, 109);
                cartuna += `<div class="col-lg-3 position-relative">
                                <div class="item m-5 text-center ingrCard w-100" data-ingredient="${ingr[i].strIngredient}">
                                    <i class="fa-solid fa-drumstick-bite ingr text-white ms-3"></i>
                                    <h2 class="h1 text-white">${ingr[i].strIngredient}</h2>
                                    <p class="text-white ">${description}</p>
                                </div>
                            </div>`;
            }
            document.getElementById('Ingredient').innerHTML = cartuna;
            ClickEvent();
        }

        let meals = [];
        async function getApiMeals1(ingredient) {
            let myData = await fetch(`https://themealdb.com/api/json/v1/1/filter.php?i=${ingredient}`);
            let response = await myData.json();
