...
function getCat(){
    ...
    for(let i=0;i<cat.length;i++){
        let description=cat[i].strCategoryDescription;
        description=description.substring(0,120)
cartuna+=`    <div class="col-lg-3 position-relative overflow-hidden">
                        <div class="card" data-cat="${cat[i].strCategory}">
                    <img src="${cat[i].strCategoryThumb}" class="card-img-top" alt="meal">
                     <div class="text-center layer bg-opacity-75 bg-white position-absolute start-0 end-0 bottom-0 top-0 rounded-2">
                    <p class="h2">${cat[i].strCategory}</p>
                    <p>${description}</p>
                  </div>
                  </div>
    ...
    document.getElementById('cat').innerHTML=cartuna;
    ClickEvent();

}
let meals = [];
async function getApiMeals3(cat) {
    let myData = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${cat}`);
    let response = await myData.json();
